...
def _ensure_source_file(db, meeting) -> uuid.UUID:
    """live_recording이면 .pcm 파일을 workspace_files에 등록하고 meeting에 연결한다."""
...
def meeting_postprocess_node(state: MeetingPostprocessState) -> dict:
    ...
    try:
        ...
        try:
            ...
            meeting_crud.update_meeting_status(db, meeting_id, status="failed")
            return {"error": f"원본 파일 등록 실패: {repr(e)}"}
        ...
        load_result = load_document(db, file_id, transcription=transcription)
        if load_result.get("status") != "success":
            print(f"[meeting_postprocess] 세그먼트 임베딩 실패: {load_result}")
