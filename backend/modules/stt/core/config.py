...
# 클라이언트가 "완전 무음"(값이 정확히 0)만 보내는 상태가 이만큼 이어지면 알린다.
#
# 왜 필요한가 (2026-08-13 실측): 회의 도중 11초가 통째로 비었는데 아무도 몰랐다.
# 연결은 살아 있고 프레임도 계속 도착하는데 내용물만 0이라 기존 경고가 전부 비켜갔다
# (no_speech는 말이 15초쯤 쌓여야 판단하는데 앞뒤 청크엔 말이 있었다).
# 최근 회의 6건 중 5건에 같은 현상이 있었고 한 건은 51초 중 45초가 비었다.
# 2초면 사람이 알아채고 고칠 수 있을 만큼 이르면서, 발화 사이 정적으로 오인할 만큼
# 짧지도 않다(정적은 잡음 바닥이 있어 애초에 0이 아니다).
AUDIO_GAP_WARN_SEC = float(os.getenv("AUDIO_GAP_WARN_SEC", "2.0"))
# 회의록 메타에 기록할 최소 공백 길이. 경고보다 낮게 잡아 짧은 끊김도 남긴다 —
# 성능을 채점할 때 "이 회의는 몇 초가 비어 있었나"를 알아야 숫자를 바로 읽는다.
AUDIO_GAP_MIN_RECORD_SEC = float(os.getenv("AUDIO_GAP_MIN_RECORD_SEC", "0.5"))
...
#
# 왜 있는가 (2026-08-18):
#   위 모델은 영어(VoxCeleb)로 학습돼서 한국어 회의 음성에서 EER 9.68%다(영어 기준 1% 안팎).
#   AI-Hub 회의 음성 20.7만 발화로 파인튜닝하니 **7.63%**로 내려갔다.
#   같은 회의 안에서 만든 시험 쌍으로 쟀기 때문에 "녹음 환경을 외운" 결과가 아니다 —
#   같은 사람 평균만 0.652→0.681로 오르고 다른 사람 평균은 0.173 그대로였다.
#
#   이 값이 중요한 이유: 문턱 조정이 네 번 다 실패한 원인이 **하한 0.35가 이미 이 모델의
#   EER 지점**이었기 때문이다(EER 문턱 0.340). 곡선 위를 오가는 것으로는 못 이기고
#   곡선 자체를 옮겨야 하는데, 그게 이 파인튜닝이다.
#
# ⚠️ 이 값을 바꾸면 **기존 목소리 프로필이 전부 무효다.** 임베딩 공간이 달라져서
#    옛 프로필과 새 임베딩을 비교하는 것은 의미가 없다. 사람이 다시 녹음할 필요는 없고
