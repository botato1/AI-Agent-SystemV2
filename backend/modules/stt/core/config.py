...
#
# 실측 분포: 명단 안 평균 0.452(하위 5% 0.202) / 명단 밖 평균 0.193(상위 5% 0.269).
# ⚠️ 이 값은 등록 프로필 품질에 직접 묶인다 — 프로필이 부실한 사람이 있으면
#    그 사람에 맞춰 바닥을 낮게 깔게 되고, 그만큼 명단 밖이 새어든다.
#    프로필을 고친 뒤 probe_out_of_roster.py로 다시 재고 올릴 것.
SPEAKER_ABSOLUTE_FLOOR = float(os.getenv("SPEAKER_ABSOLUTE_FLOOR", "0.25"))
