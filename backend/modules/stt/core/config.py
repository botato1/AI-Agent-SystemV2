...
#   0.35 → 명단 밖 100% 차단, 명단 안 95% 유지
#   0.30 → 명단 밖  90% 차단, 명단 안 98% 유지   ← 채택
#
# 0.35를 쓰다가 0.30으로 내렸다. 실제 재분석에서 0.35가 가동현의 답변 한 줄을
# 미상으로 떨어뜨렸다(그가 재등록 안 한 유일한 사람이라 가장 약하다 — 0.464).
# **두 오류의 무게가 다르다**: 명단 밖 사람이 말하는 건 드문 일이고, 참석자 발언을
# 놓치는 건 회의마다 일어난다. 차단률 10%를 내주고 유지율 3%를 사는 편이 낫다.
#
...
SPEAKER_ABSOLUTE_FLOOR = float(os.getenv("SPEAKER_ABSOLUTE_FLOOR", "0.30"))
