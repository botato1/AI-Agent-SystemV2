...
# initial_prompt(hotwords) — Whisper에게 이런 단어가 나올 가능성이 높다고 미리 힌트를 줘서
# 특이/전문 어휘 인식률을 끌어올림. 학습 없이 즉시 적용 가능한 가장 저비용 개선책.
# 실측 테스트에서 "웅얼거리다"→"우울거리다", "환각"→관련 오인식이 있었던 걸 반영해 추가.
REALTIME_INITIAL_PROMPT = (
    "비고 프로젝트 팀 회의. 용어: 환각, 웅얼거리다, 침묵, 화자분리, 모순 감지, 결정 사항, "
    "액션 아이템, API, 파이프라인, 컨테이너, 배포, 리팩토링, 레이턴시, 옵저버빌리티, GPU, 임베딩"
)
