...
# 몇 명이 동시에 말해야 "화자를 정할 수 없다"고 볼지.
#
# 2명으로 두면 안 된다 — 한 사람이 말하는 중에 누가 짧게 맞장구치는 것까지 전부 걸린다.
# 그런 구간은 주된 화자가 명확한데 이름을 지우면 손해다. 실측에서 2명 기준을 쓰자
# 오배정 1개(전원이 말한 "네 좋습니다")를 고치는 대신 맞는 이름 6개를 잃었다.
# 정말로 한 명을 고를 수 없는 것은 여러 명이 한꺼번에 말한 경우다.
OVERLAP_MIN_SPEAKERS = int(os.getenv("OVERLAP_MIN_SPEAKERS", "3"))
