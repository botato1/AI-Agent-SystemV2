...
# ── 외래어 판별 (양성 필터) ────────────────────────────────
#
# ⛔ 2026-08-14: 이 필터는 **개발자 타깃을 전제로 짜였고, 타깃이 철회되면서 알려진
#    결함이 됐다.** 동작은 그대로 두되(별도 검증이 필요한 설계 변경이므로) 기록해둔다.
#
#    측정한 결과 — 한자어 도메인 용어를 구조적으로 못 받는다:
#      · 정적 목록에 남긴 회의 어휘 9개 중 8개 탈락
#        (재논의·번복·결정사항·안건·시연·회의록·합의·보류 — '상태값'만 통과)
#      · 타 도메인 용어 20개 중 17개 탈락
#        (임상시험·항소·수강신청·보험료·납기·처방·계약서·매출·손익·결재 …)
#
#    즉 지금 이 장치는 "개발팀 회의에서 외래어 기술 용어를 줍는" 용도로만 작동한다.
#    일반 회의를 대상으로 하려면 판별 기준을 다시 세워야 한다. 후보:
#      ① 빈도 기반 — 일반 코퍼스 대비 이 회의 시리즈에서 유난히 자주 나오는 말
#         (도메인 무관하고, 우리가 이미 가진 데이터로 가능)
#      ② 사람이 고친 세그먼트(user_edited)에서 나온 단어를 우선 — 이미 가드 ①이
#         쓰는 신호이고, "실제로 틀렸던 말"이라는 근거가 붙는다
#    ①이 원칙적으로 맞지만 일반 코퍼스 기준선이 필요하다. ②는 지금 데이터로 바로 된다.
#
# 아래는 개발자 타깃 시절의 설계 근거다 (판별 기준을 다시 세울 때 참고):
#
