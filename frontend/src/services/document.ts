...
/**
 * 1-1. 문서/그래프 실시간 이벤트 스트림용 WebSocket 티켓 발급 API
 * (GET /api/workspaces/{workspace_id}/documents/stream/ticket)
 * 1회용 티켓이라 WebSocket 연결 직전에 매번 새로 발급받아야 함
 */
export interface GetDocumentStreamTicketResponse {
  status: "success" | "error";
...
export async function getDocumentStreamTicketApi(
  workspaceId: string
): Promise<GetDocumentStreamTicketResponse> {
  const API_BASE_URL = import.meta.env.VITE_API_URL || "";
  const token = localStorage.getItem("access_token");
  ...
  try {
    const response = await authFetch(
      `${API_BASE_URL}/api/workspaces/${workspaceId}/documents/stream/ticket`,
      { method: "GET" }
    );
    ...
    console.error("getDocumentStreamTicketApi error:", error);
    return {
      status: "error",
      wsTicket: null,
      message: "서버와 통신할 수 없습니다.",
